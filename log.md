...
**Action:**
- **Created** `Strategy/Prospecting/Cantus-Prospecting-Framework.md` (~5,500 words) — the v2.0 canonical prospecting framework. Replaces the three-archetype tenant split with the v2 six-demand-vector taxonomy. Standardized four-dimension tenant rubric (Power Pull 40% / Timing 25% / Counterparty Strength 20% / Geographic & Site Fit 15%) applied per vector with vector-specific signal calibrations. Two v2-native multipliers added: Multi-Vector Exposure (1.00–1.30) and Vertical-Integration Trajectory (1.00–1.25). Composite = base × multipliers. Tesla scored as multi-vector vertical-integrator anchor (Composite Tier A++); Celestica scored as Supply Chain Manufacturing Tier A with updated power-density profile. Partner Scoring preserved from v1.0 with v2 sub-industry alignment; Machine OEMs (Power Machines) added as new partner sub-category with NVIDIA flagged as priority gap. Integrated supply-demand matching exercises for Cantus campuses (three use cases including a Tesla-style 800 MW solar vertical-integration campus).
- **Created** `Strategy/Prospecting/Cantus-Prospecting-Playbook.md` (~4,500 words) — the operational sourcing manual. Per-vector signal sourcing index (where to find each Power Pull signal for each of six vectors). Per-partner-class sourcing index. Source hierarchy across SEC filings (10-K segment reporting, customer concentration footnotes, Properties section, MD&A), earnings calls, federal program databases (LPO, CHIPS, DPA, BARDA, IBAS, IRA), trade press (Data Center Dynamics, Mission Critical, Light Reading, The Information, BNEF, Synergy, Dell'Oro, etc.), regulatory data (ERCOT, ISO queues, state PUCs, Texas Comptroller, EPA permits), workforce data (BLS, LinkedIn, IBEW), and capital partner data (Form PF/ADV, PERE, sovereign wealth reports). New v2 signal sources documented: NVIDIA Roadmap / GPU Power-Density Tracker (with A100 → Rubin progression table), IRA/CHIPS/DPA/DOE LPO unified tracker, ERCOT SB 6 large-load registration tracker, vertical-integration announcement tracker. Diligence protocols (initial screen / deep dive / outreach prep / engagement progression). CII data asset cross-references. Excel workbook spec for the Prospect Tracker. Quarterly framework calibration cadence.
- **Updated** `index.md` — added Prospecting section under Strategy with both pages.

**Cross-links added:** Both new pages cross-reference [[Strategy/Cantus-Power-Sector-Thesis]], [[Strategy/Cantus-Business-Plan]], [[Strategy/Cantus-Markets]], [[Strategy/Power-Sector/Power-Industries]], [[Strategy/Cantus-Customer-Category]], [[Products/CII-Iteration-Document]], [[Strategy/Foundations/CII-DealOS-Foundation]], and [[_parameters]] (Tenant Power Price Tolerance, Capital Partner Categories, Strategic OEM Relationships, active pipeline projects).

**Audience:** capital_partner. The Framework and Playbook are designed to be the institutional discipline that the team operates from; can be selectively shared with capital partners as part of underwriting-rigor articulation.

**Notes:**
- The v0.1 → v2.0 evolution preserves the operational scoring discipline (weights, 0–5 scale, tier bands, MW heuristics, source hierarchy) while restructuring around the v2 Power-sector framework. Box v0.1 was strong on prospecting mechanics but weak on architectural alignment with v2; this update closes that gap.
- The two new multipliers (Multi-Vector Exposure + Vertical-Integration Trajectory) operationalize the "frontier company becoming selectively asset-heavy" claim from the Power Sector Thesis. Without them, a firm like Tesla scores Tier B under any single archetype; with them, Tesla emerges as a structural Tier A++ anchor matching the actual engagement priority.
- The Celestica calibration drove the upward revision of MW heuristics in the EMS / AI server assembly sub-segment (from v0.1's 40–120 MW to v2.0's 40–300+ MW). The NVIDIA Power-Density Tracker is now a standing data feed because this calibration shifts faster than annual scoring would catch.
- NVIDIA is flagged in both the Framework (§VII Reference Partner Cohort) and Playbook (§V.A) as the most strategically consequential Machine OEM relationship for Cantus, and as a priority gap in [[_parameters]] Strategic OEM Relationships. This should be elevated to Cantus Technology corporate-relationship priority.
- Open questions surfaced for future iteration include: Sovereign Compute prospect class (foreign-government / sovereign-wealth-backed compute deployments); Cantus Compute self-tenant treatment (internal counterparty scoring); Multi-vector multiplier curve fit after 2–3 quarters of scoring runs; Vertical-integration signal automation through CII / DealOS Foundation.
- Next operational steps (not built in this batch): apply the Framework to the active pipeline (Mathis McDonald, GoodPeak, Joule, Palangana, Wilmer, T1, Singleton) as deal-architecture documents; build the Excel Prospect Tracker workbook per the §VIII.A spec; produce a quarterly Prospecting Calibration Memo.
- No NPI material crossed. Cantus Forge not referenced. Hopkins not signed; author of record Dom Espinosa throughout.
